/**
 * Broadcast Status component — shows "devices reached / total".
 */
...
export default function BroadcastStatus({
  ...
  // Calculate stats per latest alert
  const latestAlert = alerts[0];
  const alertAcks = latestAlert
    ? acknowledgments.filter((a) => a.alert_id === latestAlert.id)
    : [];
    ...
    <div className="flex flex-col h-full" id="broadcast-status-panel">
      ...
      <div className="glass-panel p-6 mb-4 text-center">
        ...
        <div className="text-xs text-surface-500 mt-1">
          {reachPercent.toFixed(1)}% coverage
        </div>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-surface-500 font-mono mt-0.5">
                  {new Date(alert.timestamp).toLocaleTimeString()} · {thisAcks.length} devices
                </p>
              </div>
              <span
                className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${
                  alert.severity === 'critical'
                    ...
                    : 'bg-raksha-500/20 text-raksha-400'
                }`}
              >
                {alert.severity}
              </span>
