 ...
 * Displays zones with risk-based colors, shelters, and volunteers.
 */
    ...
    html: `
      ...
      ">
        <div style="
          ...
          border-radius: 50%;
          animation: ${risk >= 60 ? 'pulse 2s infinite' : 'none'};
        "></div>
        <div style="
          position: relative;
          width: 24px;
  ...
  return L.divIcon({
    ...
    html: `
      <div style="
        width: 32px;
        height: 32px;
        background: linear-gradient(135deg, #0047e1, #3d84ff);
        border: 2px solid #7babff;
        border-radius: 8px;
        display: flex;
        align-items: center;
        justify-content: center;
...
function parseShelterLocation(location: string): [number, number] | null {
  // Try "POINT(lon lat)" format from PostGIS
...
export default function LiveMap({ zones, shelters, volunteerCount }: LiveMapProps) {
  ...
  useEffect(() => {
    ...
    zones.forEach((zone) => {
      const marker = L.marker([zone.lat, zone.lon], {
        icon: createZoneIcon(zone.risk_score),
      });

      marker.bindPopup(`
        <div style="font-family: Inter, sans-serif; padding: 4px;">
          <div style="font-size: 14px; font-weight: 700; margin-bottom: 6px; color: ${getRiskColor(zone.risk_score)}">
            Zone ${zone.id.slice(0, 8)}
          </div>
          <div style="font-size: 12px; color: #94a3b8; margin-bottom: 4px;">
            Risk Score: <span style="color: ${getRiskColor(zone.risk_score)}; font-weight: 600;">${zone.risk_score.toFixed(1)}</span>
          </div>
  ...
  useEffect(() => {
    ...
    shelters.forEach((shelter) => {
      ...
      marker.bindPopup(`
        <div style="font-family: Inter, sans-serif; padding: 4px;">
          <div style="font-size: 14px; font-weight: 700; margin-bottom: 6px; color: #3d84ff;">
            🏠 Shelter ${shelter.id.slice(0, 8)}
          </div>
          <div style="font-size: 12px; color: #94a3b8; margin-bottom: 2px;">
            Capacity: <span style="font-weight: 600; color: white;">${shelter.capacity}</span>
          </div>
          <div style="font-size: 12px; color: #94a3b8; margin-bottom: 4px;">
            Available: <span style="font-weight: 600; color: ${occColor};">${shelter.available_beds}</span>
          </div>
          ...
          {[
            ...
            <div key={level.label} className="flex items-center gap-2 text-xs">
              <div
                ...
                style={{ backgroundColor: level.color, boxShadow: `0 0 6px ${level.color}60` }}
              />
